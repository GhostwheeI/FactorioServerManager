...
- ⚙️ **Easy Configuration:** Edit `server-settings.json` directly with Notepad through the manager. The script dynamically loads `server-settings.json` and `server-adminlist.json` from the repository root into the game on startup if they exist.
